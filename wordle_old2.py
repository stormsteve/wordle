...
def process_accurate_logic_single_threaded(all_guesses:WordSet, words:WordSet) -> dict[str, float]:
    ...
    for guess in all_guesses:
        ...
        if score <= min_score:
            min_score = score
            scores[guess] = score
            logging.debug('guess=%s score=%.3f', guess, score)
    return scores

def process_accurate_logic(
        all_guesses:WordSet, words:WordSet) -> dict[str, float]:
    """
    ...
    # Get ready to process the possible guesses using child processes
    ...
    # Pull the resulting guess scores from the queue
    last_log = False
    ...
    while True:
        try:
            ...
            logging.debug('guess=%s score=%.3f', guess, scores[guess])
        except KeyboardInterrupt:
            ...
            try:
                ...
                if not last_log:
                    last_log = True
                    if (num_alive < max_child_processes and cpu < MAX_CPU_PERCENT * 0.67):
                        proc_num += 1
...
def _log_a_score(results: list[str], scores: dict[str, float], idx: int) -> None:
    ...
    """
    if len(results) > idx:
        logging.info('accurate score for %s: %.3f', results[idx], scores[results[idx]])
...
def best_guess(dictionary: WordSet, words:WordSet, clues: Clues) -> tuple[str, Logic]:
    ...
    # Use the cache file if it will help
    ...
    # Get a list of guesses and their "score"
    scores = process_accurate_logic(all_guesses, words)

    # Pick the guess with the best score
    results = sorted(scores, key = scores.__getitem__)
    result = results[0]

    # Show the best few guesses if logging is on
    _log_a_few_scores(results, scores)

    logic.update('accurate', scores[result], words) #set(scores.keys()))
    return (result, logic)
...
class ModeAdvise(Mode):
    ...
    def get_user_guess(dictionary: WordSet, guess: str, words: WordSet,
        ...
        score = accurate(words, user_guess, 1E99)
        logic.update('user input', score, words)
        logging.info('guess=%s score=%.3f', user_guess, score)
        return user_guess
...
def game_loop() -> None:
    ...
    display_board(clues, answer if answer else guess)
